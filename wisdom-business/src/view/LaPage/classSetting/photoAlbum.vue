  ...
  <div class="classificationOfFood">
    <div class="fied_top">
      <el-button type="primary"
                 size="medium"
                 @click="addDialog"
                 v-if="this.$route.query.type">添加分类</el-button>
    </div>
    <div class="fied_con">
      <div class="bot_btn">
...
export default {
  ...
  methods: {
    create () {
      this.$newApi.XCgetCategoryPageList({
    ...
    selectDelchange() {
      ...
      this.$newApi.XCbatDelCategoryItem({
        uid: arr,
        token: JSON.parse(this.$store.state.token).token,
      }).then(val => {
        this.$message({
    ...
    copy(row) {
      this.$newApi.XCcopyCategoryItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
