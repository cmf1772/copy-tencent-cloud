...
export default {
  ...
  methods: {
    create () {
      this.$newApi.CGSPgetCategoryPageList({
    ...
    selectDelchange() {
      ...
      this.$newApi.CGSPbatDelCategoryItem({
        uid: arr,
        token: JSON.parse(this.$store.state.token).token,
      }).then(val => {
        this.$message({
    ...
    createLevelCat() {
      this.$newApi.CGSPcreateLevelCat({
        token: JSON.parse(this.$store.state.token).token,
      }).then(val => {
        this.$message({
          message: val.data.msg
    ...
    del(row) {
      this.$newApi.CGSPdelCategoryItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.$message({
    ...
    edit(row) {
      ...
      this.$newApi.CGSPgetCategoryItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.form = res.data
        ...
        if(res.data.wapcategory_file1 !== '') {
          this.fileListTwo.push({name: res.data.wapcategory_file1, url: res.data.wapcategory_file1})
          this.uploadPicUrlTwo = res.data.wapcategory_file1
        }
    ...
    copy(row) {
      this.$newApi.CGSPcopyCategoryItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
    ...
    save() {
      ...
      if(this.form.uid) {
        this.$newApi.CGSPsetCategoryItem({
          uid: this.form.uid,
          ...
          wapcategory_file1: this.uploadPicUrlTwo,
          att_name: att_name,
          att_value: att_value,
          token: JSON.parse(this.$store.state.token).token,
        }).then(res => {
        ...
        this.$newApi.CGSPaddCategoryItem({
          category_id: this.form.category_id,
          category_name: this.form.category_name,
          category_key: this.form.category_key,
          category_desc: this.form.category_desc,
