...
export default {
  ...
  methods: {
    create () {
      this.$newApi.CSGLgetCategoryPageList({
    ...
    selectDelchange() {
      ...
      this.$newApi.CSGLbatDelCategoryItem({
        uid: arr,
        token: JSON.parse(this.$store.state.token).token,
      }).then(val => {
        this.$message({
    ...
    createLevelCat() {
      this.$newApi.CSGLcreateLevelCat({
        token: JSON.parse(this.$store.state.token).token,
      }).then(val => {
        this.$message({
          message: val.data.msg
    ...
    del(row) {
      this.$newApi.CSGLdelCategoryItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.$message({
    ...
    edit(row) {
      ...
      this.$newApi.CSGLgetCategoryItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.form = res.data
    ...
    copy(row) {
      this.$newApi.CSGLcopyCategoryItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.$message({
    ...
    save() {
      ...
      if(this.dialogVisibleTitle == '编辑分类') {
        this.$newApi.CSGLsetCategoryItem({
          uid: this.form.uid,
        ...
        this.$newApi.CSGLaddCategoryItem({
          category_id: this.form.category_id,
          category_name: this.form.category_name,
          category_key: this.form.category_key,
          category_desc: this.form.category_desc,
        ...
        this.$newApi.CSGLbatAddParametersItem({
          category_id: this.form.category_id,
