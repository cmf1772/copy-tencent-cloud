...
export default {
  ...
  methods: {
    create () {
      this.$newApi.HYgetCategoryPageList({
    ...
    selectDelchange() {
      ...
      this.$newApi.HYbatDelCategoryItem({
        uid: arr,
        token: JSON.parse(this.$store.state.token).token,
      }).then(val => {
        this.$message({
    ...
    copy(row) {
      this.$newApi.HYcopyCategoryItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
