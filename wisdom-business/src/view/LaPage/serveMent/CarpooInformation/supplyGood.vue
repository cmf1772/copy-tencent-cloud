...
export default {
  ...
  methods: {
    create() {
      this.$newApi.FWgetWantSupplyPageList({
    ...
    editDis(row) {
      ...
      this.$newApi.FWgetWantSupplyItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.form = res.data
    ...
    save() {
      this.$newApi.FWsetWantSupplyItem({
        uid: this.uid,
