...
export default {
  ...
  methods: {
    create() {
      this.$newApi.PCgetOrderPageList({
    ...
    goOrder (row) {
      this.$router.push('/shopOrder/orderDetails?uid=' + row.uid + '&api=PCgetOrderItem')
    },

    editor (index, row) {
      this.dialogVisible = true
      this.$newApi.PCgetOrderItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.formData = res.data
        this.orderData = res.data.order_info
        this.orderData.sh_uid = this.orderData.sh_uid
        this.arr_goods.push(res.data.arr_goods[92])
      })
    },

    checkTrackQueryFun(index, row) {
       this.$newApi.PCdelOrderItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.$message({
