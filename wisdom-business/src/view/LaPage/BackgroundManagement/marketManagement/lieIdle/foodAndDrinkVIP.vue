...
export default {
  ...
  methods: {
    create() {
      this.$newApi.XZgetWantSupplyPageListVIP({
    ...
    save() {
      this.$newApi.XZsetWantSupplyItemVIP({
        uid: this.form.uid,
    ...
    addNav (row) {
      this.$newApi.XZgetWantSupplyItemVIP({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.form = res.data
