...
export default {
  ...
  methods: {
    create () {
      this.$newApi.LRgetCategoryPageList({
    ...
    selectDelchange() {
      ...
      this.$newApi.LRbatDelCategoryItem({
        uid: arr,
        token: JSON.parse(this.$store.state.token).token,
      }).then(val => {
        this.$message({
    ...
    copy(row) {
      this.$newApi.LRcopyCategoryItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.$message({
