...
export default {
  ...
  methods: {
    create() {
      this.$newApi.LRgetOrderPageList({
    ...
    goOrder (row) {
      this.$router.push('/shopOrder/orderDetails?uid=' + row.uid + '&api=LRgetOrderItem')
    },

    editor (index, row) {
      this.dialogVisible = true
      this.$newApi.LRgetOrderItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
