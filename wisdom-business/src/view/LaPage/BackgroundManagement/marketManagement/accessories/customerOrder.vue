...
export default {
  ...
  methods: {
    create() {
      this.$newApi.FSgetOrderPageList({
    ...
    goOrder (row) {
      this.$router.push('/shopOrder/orderDetails?uid=' + row.uid + '&api=FSgetOrderItem')
    },

    editor (index, row) {
      this.dialogVisible = true
      this.$newApi.FSgetOrderItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
