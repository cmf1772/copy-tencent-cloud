...
export default {
  ...
  methods: {
    create () {
      this.$newApi.FSgetCategoryPageList({
    ...
    selectDelchange() {
      ...
      this.$newApi.FSbatDelCategoryItem({
        uid: arr,
        token: JSON.parse(this.$store.state.token).token,
      }).then(val => {
        this.$message({
    ...
    copy(row) {
      this.$newApi.FScopyCategoryItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.$message({
