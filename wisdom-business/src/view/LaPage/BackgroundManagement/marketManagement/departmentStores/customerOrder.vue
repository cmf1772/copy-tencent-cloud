...
export default {
  ...
  methods: {
    create() {
      this.$newApi.BHgetOrderPageList({
    ...
    goOrder (row) {
      this.$router.push('/shopOrder/orderDetails?uid=' + row.uid + '&api=BHgetOrderItem')
    },

    editor (index, row) {
      this.dialogVisible = true
      this.$newApi.BHgetOrderItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
