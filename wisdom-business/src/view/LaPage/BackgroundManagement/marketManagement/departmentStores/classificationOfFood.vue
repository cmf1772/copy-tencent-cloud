...
export default {
  ...
  methods: {
    create () {
      this.$newApi.BHgetCategoryPageList({
    ...
    selectDelchange() {
      ...
      this.$newApi.BHbatDelCategoryItem({
        uid: arr,
        token: JSON.parse(this.$store.state.token).token,
      }).then(val => {
        this.$message({
    ...
    copy(row) {
      this.$newApi.BHcopyCategoryItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.$message({
