...
export default {
  ...
  methods: {
    create () {
      this.$newApi.getCategoryPageListVIP({
    ...
    selectDelchange() {
      ...
      this.$newApi.batDelCategoryItemVIP({
        uid: arr,
        token: JSON.parse(this.$store.state.token).token,
      }).then(val => {
        this.$message({
    ...
    del(row) {
      this.$newApi.delCategoryItemVIP({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.$message({
    ...
    edit(row) {
      ...
      this.$newApi.getCategoryItemVIP({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.form = res.data
    ...
    copy(row) {
      this.$newApi.copyCategoryItemVIP({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.$message({
    ...
    save() {
      ...
      if(this.form.uid) {
        this.$newApi.setCategoryItemVIP({
        uid: this.form.uid,
        ...
        this.$newApi.addCategoryItemVIP({
          category_id: this.form.category_id,
          category_name: this.form.category_name,
          category_key: this.form.category_key,
          category_desc: this.form.category_desc,
