...
export default {
  ...
  methods: {
    create() {
      this.$newApi.getWantSupplyPageListVIP({
    ...
    save() {
      this.$newApi.setWantSupplyItemVIP({
        uid: this.form.uid,
    ...
    addNav (row) {
      this.$newApi.getWantSupplyItemVIP({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.form = res.data
