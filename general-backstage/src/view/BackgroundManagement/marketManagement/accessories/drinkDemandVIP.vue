...
export default {
  ...
  methods: {
    create() {
      this.$newApi.FSgetWantBuyPageListVIP({
    ...
    del(row) {
      this.$newApi.FSdelWantBuyItemVIP({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.$message({
    ...
    audit(row) {
      this.$newApi.FSsetCheckItemBuyVIP({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.$message({
    ...
    reject(row) {
      this.$prompt('', '驳回原因', {
        ...
        this.$newApi.FSsetBackItemBuyVIP({
          uid: row.uid,
          back_reason: value,
          token: JSON.parse(this.$store.state.token).token,
        }).then(res => {
    ...
    save() {
      this.$newApi.FSsetWantBuyItemVIP({
        uid: this.form.uid,
    ...
    addNav (row) {
      this.$newApi.FSgetWantBuyItemVIP({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.form = res.data
