...
export default {
  ...
  methods: {
    create() {
      this.$newApi.FSgetWantSupplyPageListVIP({
    ...
    save() {
      this.$newApi.FSsetWantSupplyItemVIP({
        uid: this.form.uid,
    ...
    addNav (row) {
      this.$newApi.FSgetWantSupplyItemVIP({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.form = res.data
