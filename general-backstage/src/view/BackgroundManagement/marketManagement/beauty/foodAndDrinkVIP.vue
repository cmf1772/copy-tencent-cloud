...
export default {
  ...
  methods: {
    create() {
      this.$newApi.LRgetWantSupplyPageListVIP({
    ...
    save() {
      this.$newApi.LRsetWantSupplyItemVIP({
        uid: this.form.uid,
    ...
    addNav (row) {
      this.$newApi.LRgetWantSupplyItemVIP({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.form = res.data
