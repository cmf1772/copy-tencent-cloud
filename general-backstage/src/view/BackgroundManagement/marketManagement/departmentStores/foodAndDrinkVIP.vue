...
export default {
  ...
  methods: {
    create() {
      this.$newApi.BHgetWantSupplyPageListVIP({
    ...
    save() {
      this.$newApi.BHsetWantSupplyItemVIP({
        uid: this.form.uid,
    ...
    addNav (row) {
      this.$newApi.BHgetWantSupplyItemVIP({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.form = res.data
