...
export default {
  ...
  methods: {
    create() {
      this.$newApi.BHgetWantBuyPageListVIP({
    ...
    del(row) {
      this.$newApi.BHdelWantBuyItemVIP({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.$message({
    ...
    audit(row) {
      this.$newApi.BHsetCheckItemBuyVIP({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.$message({
    ...
    reject(row) {
      this.$prompt('', '驳回原因', {
        ...
        this.$newApi.BHsetBackItemBuyVIP({
          uid: row.uid,
          back_reason: value,
          token: JSON.parse(this.$store.state.token).token,
        }).then(res => {
    ...
    save() {
      this.$newApi.BHsetWantBuyItemVIP({
        uid: this.form.uid,
    ...
    addNav (row) {
      this.$newApi.BHgetWantBuyItemVIP({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.form = res.data
