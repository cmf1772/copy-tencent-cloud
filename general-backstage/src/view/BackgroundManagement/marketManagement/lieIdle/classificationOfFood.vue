...
export default {
  ...
  methods: {
    create () {
      this.$newApi.XZgetCategoryPageList({
    ...
    selectDelchange() {
      ...
      this.$newApi.XZbatDelCategoryItem({
        uid: arr,
        token: JSON.parse(this.$store.state.token).token,
      }).then(val => {
        this.$message({
    ...
    copy(row) {
      this.$newApi.XZcopyCategoryItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.$message({
