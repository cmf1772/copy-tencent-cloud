...
export default {
  ...
  methods: {
    create() {
      this.$newApi.XZgetOrderPageList({
    ...
    goOrder (row) {
      this.$router.push('/shopOrder/orderDetails?uid=' + row.uid + '&api=XZgetOrderItem')
    },

    editor (index, row) {
      this.dialogVisible = true
      this.$newApi.XZgetOrderItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
