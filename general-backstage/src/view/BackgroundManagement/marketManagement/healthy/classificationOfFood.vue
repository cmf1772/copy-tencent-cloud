...
export default {
  ...
  methods: {
    create () {
      this.$newApi.JKgetCategoryPageList({
    ...
    selectDelchange() {
      ...
      this.$newApi.JKbatDelCategoryItem({
        uid: arr,
        token: JSON.parse(this.$store.state.token).token,
      }).then(val => {
        this.$message({
    ...
    copy(row) {
      this.$newApi.JKcopyCategoryItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.$message({
