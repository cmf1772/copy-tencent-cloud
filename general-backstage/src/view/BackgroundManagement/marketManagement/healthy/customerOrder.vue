...
export default {
  ...
  methods: {
    create() {
      this.$newApi.JKgetOrderPageList({
    ...
    goOrder (row) {
      this.$router.push('/shopOrder/orderDetails?uid=' + row.uid + '&api=JKgetOrderItem')
    },

    editor (index, row) {
      this.dialogVisible = true
      this.$newApi.JKgetOrderItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
