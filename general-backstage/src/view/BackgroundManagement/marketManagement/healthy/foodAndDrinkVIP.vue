...
export default {
  ...
  methods: {
    create() {
      this.$newApi.JKgetWantSupplyPageListVIP({
    ...
    save() {
      this.$newApi.JKsetWantSupplyItemVIP({
        uid: this.form.uid,
    ...
    addNav (row) {
      this.$newApi.JKgetWantSupplyItemVIP({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.form = res.data
