...
export default {
  ...
  methods: {
    create() {
      this.$newApi.JKgetWantBuyPageListVIP({
    ...
    del(row) {
      this.$newApi.JKdelWantBuyItemVIP({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.$message({
    ...
    audit(row) {
      this.$newApi.JKsetCheckItemBuyVIP({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.$message({
    ...
    reject(row) {
      this.$prompt('', '驳回原因', {
        ...
        this.$newApi.JKsetBackItemBuyVIP({
          uid: row.uid,
          back_reason: value,
          token: JSON.parse(this.$store.state.token).token,
        }).then(res => {
    ...
    save() {
      this.$newApi.JKsetWantBuyItemVIP({
        uid: this.form.uid,
    ...
    addNav (row) {
      this.$newApi.JKgetWantBuyItemVIP({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.form = res.data
