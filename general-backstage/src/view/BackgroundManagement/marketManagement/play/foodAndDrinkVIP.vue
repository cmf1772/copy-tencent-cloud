...
export default {
  ...
  methods: {
    create() {
      this.$newApi.WLgetWantSupplyPageListVIP({
    ...
    save() {
      this.$newApi.WLsetWantSupplyItemVIP({
        uid: this.form.uid,
    ...
    addNav (row) {
      this.$newApi.WLgetWantSupplyItemVIP({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.form = res.data
