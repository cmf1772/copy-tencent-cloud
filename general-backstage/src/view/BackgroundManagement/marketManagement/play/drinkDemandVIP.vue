...
export default {
  ...
  methods: {
    create() {
      this.$newApi.WLgetWantBuyPageListVIP({
    ...
    del(row) {
      this.$newApi.WLdelWantBuyItemVIP({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.$message({
    ...
    audit(row) {
      this.$newApi.WLsetCheckItemBuyVIP({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.$message({
    ...
    reject(row) {
      this.$prompt('', '驳回原因', {
        ...
        this.$newApi.WLsetBackItemBuyVIP({
          uid: row.uid,
          back_reason: value,
          token: JSON.parse(this.$store.state.token).token,
        }).then(res => {
    ...
    save() {
      this.$newApi.WLsetWantBuyItemVIP({
        uid: this.form.uid,
    ...
    addNav (row) {
      this.$newApi.WLgetWantBuyItemVIP({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.form = res.data
