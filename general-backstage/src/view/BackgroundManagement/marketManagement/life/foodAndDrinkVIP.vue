...
export default {
  ...
  methods: {
    create() {
      this.$newApi.SHgetWantSupplyPageListVIP({
    ...
    save() {
      this.$newApi.SHsetWantSupplyItemVIP({
        uid: this.form.uid,
    ...
    addNav (row) {
      this.$newApi.SHgetWantSupplyItemVIP({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.form = res.data
