...
export default {
  ...
  methods: {
    create () {
      this.$newApi.SHgetCategoryPageList({
    ...
    selectDelchange() {
      ...
      this.$newApi.SHbatDelCategoryItem({
        uid: arr,
        token: JSON.parse(this.$store.state.token).token,
      }).then(val => {
        this.$message({
    ...
    copy(row) {
      this.$newApi.SHcopyCategoryItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.$message({
