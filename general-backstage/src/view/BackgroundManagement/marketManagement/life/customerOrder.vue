...
export default {
  ...
  methods: {
    create() {
      this.$newApi.SHgetOrderPageList({
    ...
    goOrder (row) {
      this.$router.push('/shopOrder/orderDetails?uid=' + row.uid + '&api=SHgetOrderItem')
    },

    editor (index, row) {
      this.dialogVisible = true
      this.$newApi.SHgetOrderItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
