...
export default {
  ...
  methods: {
    create() {
      this.$newApi.QCgetWantBuyPageListVIP({
    ...
    del(row) {
      this.$newApi.QCdelWantBuyItemVIP({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.$message({
    ...
    audit(row) {
      this.$newApi.QCsetCheckItemBuyVIP({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.$message({
    ...
    reject(row) {
      this.$prompt('', '驳回原因', {
        ...
        this.$newApi.QCsetBackItemBuyVIP({
          uid: row.uid,
          back_reason: value,
          token: JSON.parse(this.$store.state.token).token,
        }).then(res => {
    ...
    save() {
      this.$newApi.QCsetWantBuyItemVIP({
        uid: this.form.uid,
    ...
    addNav (row) {
      this.$newApi.QCgetWantBuyItemVIP({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.form = res.data
