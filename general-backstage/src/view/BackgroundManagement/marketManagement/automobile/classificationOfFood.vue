...
export default {
  ...
  methods: {
    create () {
      this.$newApi.QCgetCategoryPageList({
    ...
    selectDelchange() {
      ...
      this.$newApi.QCbatDelCategoryItem({
        uid: arr,
        token: JSON.parse(this.$store.state.token).token,
      }).then(val => {
        this.$message({
    ...
    copy(row) {
      this.$newApi.QCcopyCategoryItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.$message({
