...
export default {
  ...
  methods: {
    create () {
      this.$newApi.FCgetCategoryPageList({
    ...
    selectDelchange() {
      ...
      this.$newApi.FCbatDelCategoryItem({
        uid: arr,
        token: JSON.parse(this.$store.state.token).token,
      }).then(val => {
        this.$message({
    ...
    copy(row) {
      this.$newApi.FCcopyCategoryItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.$message({
