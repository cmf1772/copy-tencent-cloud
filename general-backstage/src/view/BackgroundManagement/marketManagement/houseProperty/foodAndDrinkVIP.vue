...
export default {
  ...
  methods: {
    create() {
      this.$newApi.FCgetWantSupplyPageListVIP({
    ...
    save() {
      this.$newApi.FCsetWantSupplyItemVIP({
        uid: this.form.uid,
    ...
    addNav (row) {
      this.$newApi.FCgetWantSupplyItemVIP({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.form = res.data
