...
export default {
  ...
  methods: {
    create() {
      this.$newApi.FCgetOrderPageList({
    ...
    goOrder (row) {
      this.$router.push('/shopOrder/orderDetails?uid=' + row.uid + '&api=FCgetOrderItem')
    },

    editor (index, row) {
      this.dialogVisible = true
      this.$newApi.FCgetOrderItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
