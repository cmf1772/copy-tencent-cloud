...
export default {
  ...
  methods: {
    create () {
      this.$newApi.JZgetCategoryPageList({
    ...
    selectDelchange() {
      ...
      this.$newApi.JZbatDelCategoryItem({
        uid: arr,
        token: JSON.parse(this.$store.state.token).token,
      }).then(val => {
        this.$message({
    ...
    copy(row) {
      this.$newApi.JZcopyCategoryItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
