...
export default {
  ...
  methods: {
    create () {
      this.$newApi.JPgetCategoryPageList({
    ...
    selectDelchange() {
      ...
      this.$newApi.JPbatDelCategoryItem({
        uid: arr,
        token: JSON.parse(this.$store.state.token).token,
      }).then(val => {
        this.$message({
    ...
    copy(row) {
      this.$newApi.JPcopyCategoryItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
