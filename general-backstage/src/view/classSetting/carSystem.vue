...
export default {
  ...
  methods: {
    create () {
      this.$newApi.PPCgetCategoryPageList({
    ...
    selectDelchange() {
      ...
      this.$newApi.PPCbatDelCategoryItem({
        uid: arr,
        token: JSON.parse(this.$store.state.token).token,
      }).then(val => {
        this.$message({
    ...
    del(row) {
      this.$newApi.PPCdelCategoryItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.$message({
    ...
    edit(row) {
      ...
      this.$newApi.PPCgetCategoryItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        this.form = res.data
    ...
    copy(row) {
      this.$newApi.PPCcopyCategoryItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
    ...
    save() {
      ...
      if(this.form.uid) {
        this.$newApi.PPCsetCategoryItem({
          uid: this.form.uid,
        ...
        this.$newApi.PPCaddCategoryItem({
          category_id: this.form.category_id,
          category_name: this.form.category_name,
          // category_key: this.form.category_key,
          // category_desc: this.form.category_desc,
