...
export default {
  ...
  methods: {
    create () {
      this.$newApi.JYgetCategoryPageList({
    ...
    selectDelchange() {
      ...
      this.$newApi.JYbatDelCategoryItem({
        uid: arr,
        token: JSON.parse(this.$store.state.token).token,
      }).then(val => {
        this.$message({
    ...
    copy(row) {
      this.$newApi.JYcopyCategoryItem({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
