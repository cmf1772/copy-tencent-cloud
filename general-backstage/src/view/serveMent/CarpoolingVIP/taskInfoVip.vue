...
export default {
  ...
  methods: {
    create() {
      this.$newApi.QGgetWantBuyPageListVIP({
    ...
    delDis(row) {
      this.$newApi.QGdelWantBuyItemVIP({
        uid: row.uid,
        token: JSON.parse(this.$store.state.token).token,
      }).then(res => {
        if(res.data.err_code) {
    ...
    save() {
      this.$newApi.QGsetWantBuyItemVIP({
        uid: this.uid,
